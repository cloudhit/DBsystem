...
Status BTreeFile::insert(const void *key, const RID rid) {
  // put your code here
  //cout<<rid.pageNo<<" "<<rid.slotNo<<endl;
  if(root_page->root == -1){
    Page* page;
    PageId pageid;
    ...
    if(tmp->get_type() == LEAF){
      ...
      //int oa;
      //memcpy(&oa, result_key, root_page->keysize);
      //cout<<"insertkey"<<oa<<endl;
...
Status BTreeFile::Delete(const void *key, const RID rid) {
  ...
  while(((SortedPage*)page)->get_type() == INDEX){
    ...
    //cout<<tmp->curPage<< " "<<tmp->getLeftLink()<<endl;
    PageId pageno;
  ...
  RID dataRid;
  //Status st = leaf->get_data_rid((void*)key, root_page->key_type, dataRid);
  //if(st!= OK) cout<<"ouyi is life winner and a bug";
  bool found = 0;
  ...
  //if(st != OK) MINIBASE_FIRST_ERROR(BTREE, LOSTTOFIND);
  MINIBASE_BM->unpinPage(leaf->curPage, TRUE);
  return st;
}
